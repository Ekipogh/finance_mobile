...
@GenerateMocks([ExpenseCategory])
main() {
  testWidgets('Test plus button tapping and New Expense Page appearing',
      (WidgetTester tester) async {
    await ExpenseCategory(name: "foobar").save();
    Widget financeApp = FinanceApp();
    ...
    expect(find.text("Please create some categories"), findsOneWidget);
    //expect(find.byKey(Key("expenseCategoryDropDown"), skipOffstage: false),
    //    findsOneWidget);
  ...
  testWidgets('Test adding new Expense', (WidgetTester tester) async {
    ...
    await tester.enterText(find.byKey(Key("expenseAmountField")), "1234567");
